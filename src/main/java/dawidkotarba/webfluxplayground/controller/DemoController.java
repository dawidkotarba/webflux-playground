...
public class DemoController {
    ...
    @GetMapping(path = "/fluxIntegerSequenceNoDelay", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    ...
    // Sequential retrieveal every second
    @GetMapping(path = "/fluxIntegerSequence", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    ...
    @GetMapping(path = "/coldPublisher", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<Integer> coldPublisher() {
        return coldPublisher;
    }

    // http://localhost:8080/hotPublisher
    @GetMapping(path = "/hotPublisher", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<Integer> hotPublisher() {
        return hotPublisher;
    }

    // http://localhost:8080/hotPublisherFromCold
    @GetMapping(path = "/hotPublisherFromCold", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<Integer> hotPublisherFromCold() {
        return hotPublisherFromCold;
    }

    // http://localhost:8080/zipPublishers
    @GetMapping(path = "/zipPublishers", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
